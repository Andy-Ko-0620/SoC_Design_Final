# 0 "fir.c"
# 1 "/home/andy/SoC_Design/SoC_Design_Final/qs/testbench/counter_la_fir//"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "fir.c"
# 1 "../../firmware/defs.h" 1
...
void __attribute__ ( ( section ( ".mprjram" ) ) ) initqs() {
  ...
  *(uint32_t *)(&(*(volatile uint32_t*)0x30000040) + i) = A[i];
 }

 (*(volatile uint32_t*)0x30000010) = 10;
}

int* __attribute__ ( ( section ( ".mprjram" ) ) ) qsort(){


 while ((*(volatile uint32_t*)0x30000000) & 4 == 0) continue;
 (*(volatile uint32_t*)0x30000000) = 1;

 for (int i = 0; i < 10; i++)
 {
  (*(volatile uint32_t*)0x30000080) = 5;
